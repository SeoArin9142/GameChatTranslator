...
- Gemini/Google API 호출 실패 시 번역창 상단에는 짧은 안내가 잠시 표시되고, 로그창에는 HTTP 상태 코드와 사용자가 확인할 항목이 함께 기록됩니다.
- 환경설정창의 **OCR 진단 화면 열기** 버튼이나 `Key_OcrDiagnostic` 단축키로 원본 캡처, Color/ColorThick/Adaptive 전처리 이미지, 언어별 OCR 결과, 후보 점수를 즉시 확인할 수 있습니다.
- OCR 진단 화면의 **진단 텍스트 복사** 버튼으로 요약과 후보별 OCR 결과를 바로 클립보드에 복사할 수 있습니다.
- OCR 진단 결과 ZIP에는 앱 버전, 언어 설정, OCR 모드, OCR 언어팩 설치 상태, 주요 설정 요약이 함께 저장됩니다.
- 환경설정창의 **저사양 / 빠름 / 정확도** 추천 프리셋 버튼으로 OCR 배율, 이진화 기준, 자동 번역 주기, 결과 표시 방식을 빠르게 채울 수 있습니다. 추천 프리셋은 입력칸만 바꾸며 저장 버튼을 눌러야 적용됩니다.
