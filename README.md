...
아래 환경은 현재 개발자가 빌드, 게시, 실사용 테스트에 사용한 기준 시스템입니다. CPU-Z / DxDiag 자료에서 공개 가능한 주요 사양만 추출해 정리했습니다.
...
> 참고: 위 사양은 개발/검증 기준 환경이며, 프로그램 실행 최소 사양을 의미하지 않습니다. CPU-Z / DxDiag 원본에는 PC 이름, 장치 식별자 등 민감할 수 있는 항목이 포함될 수 있으므로 저장소에는 원본 텍스트를 보관하지 않습니다.

## 📥 다운로드 (Download)

최신 버전의 실행 파일을 아래 링크에서 다운로드하세요!

[**👉 GameChatTranslator v1.0.22-alpha 다운로드 받기**](https://github.com/SeoArin9142/GameChatTranslator/releases/download/v.1.0.22-alpha/GameChatTranslator_v1.0.22-alpha.zip)

---

## 💻 시스템 요구사항

GameChatTranslator는 Windows OCR, WPF 오버레이, 화면 캡처, 온라인 번역 API를 함께 사용하는 Windows 전용 프로그램입니다. OCR 자체는 고성능 GPU를 요구하지 않지만, 게임과 동시에 실행되므로 실제 체감 성능은 실행 중인 게임 사양의 영향을 받습니다.

### 필수 조건

- Windows OCR을 사용할 수 있는 64-bit Windows 환경
- Windows OCR 언어팩 설치
- 인터넷 연결
- 관리자 권한 실행 승인
- 스트리노바 게임 클라이언트와 동일한 화면에서 캡처 가능한 환경

### 최소사양

| 항목 | 최소 기준 |
|:---|:---|
| OS | Windows 10 2004, Build 19041 이상 / 64-bit |
| CPU | 4코어 이상 |
| 메모리 | 8GB 이상 |
| GPU | DirectX 11/12 지원 내장 GPU 이상 |
| 저장공간 | 1GB 이상 여유 공간 권장 |
| 디스플레이 | 1920 x 1080 이상 권장 |
| 네트워크 | Google/Gemini 사용 시 인터넷 연결, Local LLM 사용 시 LM Studio Local Server |
...
| 저장공간 | 1GB 이상 여유 공간 |
| 디스플레이 | 1920 x 1080 이상, 멀티모니터 사용 시 최신 GPU 드라이버 권장 |
| 네트워크 | 안정적인 인터넷 연결 |

> 참고: 개발 / 검증 환경에 표시된 고성능 PC 사양은 실제 테스트 기준 환경이며, 프로그램 실행을 위한 권장사양 자체를 의미하지 않습니다.
...
- **번역 API 오류 안내**: Gemini/Google 호출 실패 시 API 키, 모델명, 할당량, 네트워크 등 확인할 항목을 로그에 표시.
- **Local LLM 번역 엔진**: LM Studio/OpenAI 호환 로컬 서버를 사용해 OCR 오타 복원형 번역을 시도하고, 실패 시 Google 번역으로 전환.
- **기본 번역 엔진 저장**: Google/Gemini/Local LLM 중 시작 엔진을 설정창에서 선택하고 재실행 후에도 유지.
- **Local LLM 연결 테스트**: 설정창에서 LM Studio 서버와 모델 ID가 맞는지 바로 확인.
...
| **번역 엔진 변경** | `Ctrl + -` | Google → Gemini → Local LLM 순환 |
...
- `GameLanguage`: OCR 기준 게임 언어입니다. Google 무료 번역에서도 원문 언어로 사용해 자동 언어 감지 오류를 줄입니다.
...
- `TranslationEngine`: 시작 시 사용할 기본 번역 엔진입니다. `Google`, `Gemini`, `LocalLlm` 중 하나이며 설정창에서 변경할 수 있습니다.
- `LocalLlmEndpoint`: LM Studio/OpenAI 호환 Local LLM chat completions 주소입니다. 기본값은 `http://localhost:1234/v1/chat/completions`입니다.
- `LocalLlmModel`: Local LLM 모델 ID입니다. 기본값은 `qwen/qwen3.5-9b`입니다.
- `LocalLlmTimeoutSeconds`: Local LLM 응답 대기 시간입니다. 기본값은 10초이며 1~60초 범위로 자동 보정됩니다.
- `LocalLlmMaxTokens`: Local LLM 번역 응답 최대 토큰 수입니다. 기본값은 160이며 40~512 범위로 자동 보정됩니다.
- `SaveDebugImages`: 디버그용 캡처 이미지를 저장할지 설정합니다. 기본값은 `false`입니다.
- 자동/수동 번역을 실행하면 `logs` 폴더에 `[OCR PERF]` 항목이 기록되며, OCR 지연 원인 분석에 사용할 수 있습니다. 로그에는 FastPath 성공/실패와 Fallback 실행 여부도 함께 남습니다.
- `Ctrl+-`는 Google → Gemini → Local LLM 순서로 번역 엔진을 전환합니다. Gemini API 키가 없으면 Gemini 단계는 건너뜁니다.
- 환경설정창의 **Local LLM 연결 테스트** 버튼은 `LocalLlmEndpoint` 기준으로 `/v1/models`를 조회해 서버 연결과 모델 ID 존재 여부를 확인합니다.
- Gemini/Google/Local LLM 호출 실패 시 번역창 상단에는 짧은 안내와 `Ctrl+=` 로그창 확인 문구가 잠시 표시되고, 로그창에는 HTTP 상태 코드와 사용자가 확인할 항목이 함께 기록됩니다.
- 환경설정창의 **OCR 진단 화면 열기** 버튼이나 `Key_OcrDiagnostic` 단축키로 원본 캡처, Color/ColorThick/Adaptive 전처리 이미지, 언어별 OCR 결과, 후보 점수를 즉시 확인할 수 있습니다.
- OCR 진단 화면의 **요약 복사 / 전체 텍스트 복사 / ZIP 저장** 버튼으로 상황에 맞게 진단 정보를 공유할 수 있습니다.
- OCR 진단 결과 ZIP에는 앱 버전, 언어 설정, OCR 모드, OCR 언어팩 설치 상태, 주요 설정 요약이 함께 저장됩니다.
- 환경설정창의 **저사양 / 빠름 / 정확도** 추천 프리셋 버튼으로 OCR 배율, 이진화 기준, 자동 번역 주기, 결과 표시 방식을 빠르게 채울 수 있습니다. 추천 프리셋은 현재값과 추천값 차이를 안내하고 입력칸만 바꾸며, 저장 버튼을 눌러야 적용됩니다.
...
- `Key_OcrDiagnostic`: OCR 진단 화면 열기 단축키입니다. 기본값은 `Ctrl+5`입니다.
- `Key_HotkeyGuideToggle`: 번역창 단축키 안내 ON/OFF 단축키입니다. 기본값은 `Ctrl+F10`입니다. 기본 상태는 숨김이며, 숨김 상태에서는 이 단축키와 자동 상태/번역기만 표시합니다.
...
## 업데이트 내역 (v.1.0.22-alpha)

   이번 버전에서는 Local LLM 번역 엔진의 실사용 편의를 보강했습니다.

   🧠 기본 번역 엔진 저장

      설정창에서 Google / Gemini / Local LLM 중 시작 번역 엔진을 선택할 수 있습니다.

      Ctrl+-로 실행 중 번역 엔진을 바꾸면 선택값을 config.ini에 저장해 다음 실행에도 유지합니다.

   🔌 Local LLM 연결 테스트

      설정창의 Local LLM 연결 테스트 버튼으로 LM Studio 서버 연결과 모델 ID 존재 여부를 확인할 수 있습니다.

      /v1/chat/completions 주소를 기준으로 /v1/models를 조회해 endpoint 또는 모델명 오류를 빠르게 확인합니다.

<details>
<summary>지난 업데이트 내역</summary>

## 업데이트 내역 (v.1.0.21-alpha)

   이번 버전에서는 OCR 성능 진단 로그를 보강해 자동 번역 튜닝에 필요한 데이터를 더 명확하게 확인할 수 있게 했습니다.

   📊 OCR 성능 진단 로그 보강

      `[OCR PERF]` 로그에 FastPath 성공/실패와 Fallback 실행 여부를 함께 기록합니다.

      자동 모드가 실제로 빠른 경로를 탔는지, 실패 후 고정확도 후보 비교로 넘어갔는지 확인할 수 있습니다.

   🪟 로그창 평균 요약 개선

      로그창 상단 OCR 평균 요약에 FastPath 성공률과 Fallback 비율을 추가했습니다.

      OCR 알고리즘 자체는 변경하지 않았고, 차후 속도/정확도 튜닝을 위한 진단 기반을 보강했습니다.

## 업데이트 내역 (v.1.0.20-alpha)

   이번 버전에서는 오류 안내, OCR 진단 화면, 추천 프리셋 UI를 더 명확하게 정리했습니다.

   ⚠️ 번역 API 오류 안내 보강

      번역창 상단 오류 안내에 `Ctrl+=` 로그창 확인 문구를 함께 표시합니다.

      짧은 안내는 게임 화면 방해를 줄이고, 상세 원인은 로그창에서 확인하도록 역할을 분리했습니다.

   🔎 OCR 진단 화면 공유 기능 정리

      OCR 진단 화면의 버튼을 [요약 복사] / [전체 텍스트 복사] / [ZIP 저장]으로 구분했습니다.

      요약 탭에 후보 비교 표를 추가해 선택 후보, 점수, 결과량을 빠르게 비교할 수 있습니다.

   ⚙️ 추천 프리셋 UI 개선

      사용자 프리셋과 내장 추천 프리셋 설명을 분리했습니다.

      추천 프리셋 적용 전 현재값에서 추천값으로 바뀌는 항목을 안내합니다.

      추천 프리셋을 눌러도 사용자 프리셋 이름 입력칸은 자동 변경하지 않도록 정리했습니다.

## 업데이트 내역 (v.1.0.19-alpha)

   이번 버전에서는 번역 API 오류 안내와 OCR 진단 결과 공유 편의성을 개선했습니다.

   ⚠️ 번역 API 오류 안내 개선

      Gemini/Google 번역 API 호출 실패 시 번역창 상단에 짧은 안내 문구를 잠시 표시합니다.

      API Key, 모델명, 할당량, 서버 오류 등 상세 원인은 기존처럼 로그창에 남겨 문제 원인을 확인할 수 있습니다.

   📋 OCR 진단 텍스트 복사 추가

      OCR 진단 화면에 [진단 텍스트 복사] 버튼을 추가했습니다.

      진단 요약과 후보별 OCR 결과를 클립보드로 복사해 이슈 제보나 비교 분석에 바로 사용할 수 있습니다.

## 업데이트 내역 (v.1.0.18-alpha)

   이번 버전에서는 OCR 진단 화면의 가독성을 높이고, 상황별 추천 설정 프리셋을 추가했습니다.

   🔎 OCR 진단 화면 UI 개선

      진단 화면 상단에 선택 후보, 점수, 처리 시간, OCR 호출 수를 요약 표시합니다.

      후보 탭에 점수를 함께 표시하고, 선택된 후보를 더 쉽게 구분할 수 있도록 정리했습니다.

      후보별 상세 화면에 후보명, 선택 여부, 점수, 결과량 요약을 추가했습니다.

   ⚙️ 추천 설정 프리셋 추가

      환경설정창에 저사양 / 빠름 / 정확도 추천 프리셋 버튼을 추가했습니다.

      추천 프리셋은 OCR 배율, 이진화 기준, 자동 번역 주기, 결과 표시 방식만 입력칸에 채웁니다.

      언어, API 키, 단축키, 캡처 영역은 변경하지 않으며 저장 버튼을 눌러야 실제 설정으로 적용됩니다.

## 업데이트 내역 (v.1.0.17-alpha)

   이번 버전에서는 OCR 진단 화면 접근성과 진단 ZIP의 분석 정보를 강화했습니다.

   🔎 OCR 진단 화면 단축키 추가

      OCR 진단 화면을 `Ctrl + 5` 기본 단축키로 바로 열 수 있도록 추가했습니다.

      환경설정창의 단축키 설정에서 OCR 진단 화면 단축키를 변경할 수 있습니다.

      단축키 초기화, 설정 저장, 프리셋 저장/불러오기에도 OCR 진단 단축키가 함께 반영됩니다.

   📦 OCR 진단 ZIP 환경 정보 보강

      OCR 진단 ZIP의 `summary.txt`에 앱 버전, 게임 언어, 번역 언어, OCR 모드, 디버그 저장 여부를 함께 기록합니다.

      표시 좌표와 물리 픽셀 좌표를 함께 남겨 DPI/멀티모니터 캡처 문제를 분석하기 쉽게 했습니다.

      Windows OCR 언어팩 설치 상태도 ZIP에 포함해 인식 실패 원인을 확인하기 쉽게 했습니다.

## 업데이트 내역 (v.1.0.16-alpha)

   이번 버전에서는 설정창 가독성, OCR 진단 결과 공유, 번역 API 오류 안내, 릴리즈 자동화 안정성을 개선했습니다.

   🧩 환경설정창 UI 정리

      설정창을 섹션별로 구분하고, 2열 레이아웃을 적용해 긴 설정 항목을 한눈에 보기 쉽게 정리했습니다.

      설정 관리 / 언어·OCR / 실행 옵션 / 번역 엔진 영역을 분리해 필요한 항목을 더 빠르게 찾을 수 있습니다.

   📦 OCR 진단 결과 ZIP 저장

      OCR 진단 화면에서 진단 결과를 ZIP 파일로 저장할 수 있도록 개선했습니다.

      요약 텍스트, 원본 캡처 이미지, 리사이즈 이미지, 후보별 전처리/크롭 이미지와 상세 정보를 함께 저장합니다.

      인식 실패 장면을 공유하거나 비교할 때 필요한 자료를 한 번에 묶을 수 있습니다.

   🧭 번역 API 오류 안내 강화

      Gemini/Google API 호출 실패 시 로그창에 HTTP 상태 코드와 확인해야 할 항목을 더 구체적으로 표시합니다.

      Gemini API 키, 모델명, 권한, 할당량, 네트워크 문제를 구분해 확인할 수 있도록 안내합니다.

      Gemini 실패 후 Google fallback으로 전환되는 경우에도 원인을 로그에 남깁니다.

   🛠️ 릴리즈 워크플로우 정리

      GitHub Actions 릴리즈 워크플로우의 주요 action 버전을 Node 24 기반 버전으로 갱신했습니다.

      이후 자동 릴리즈 빌드의 Node.js 런타임 경고를 줄일 수 있도록 정리했습니다.

      전체 단위 테스트 수가 175개로 늘어났습니다.

## 업데이트 내역 (v.1.0.15-alpha)

   이번 버전에서는 설정 입력 안정성, 문서화, 테스트 구조, 번역 API 호출부를 정리해 사용자 설정 오류와 개발 유지보수 부담을 줄였습니다.

   ⚙️ 설정창 숫자 입력 검증 강화

      OCR 이진화 기준, 자동 번역 주기, 누적 표시 줄 수 입력칸에서 숫자 외 키보드 입력과 붙여넣기를 차단합니다.

      범위를 벗어난 값을 입력하면 저장 전에 보정 예정값을 안내하도록 개선했습니다.

   📋 시스템 요구사항 문서 추가

      Windows OCR, 관리자 권한, 인터넷 연결, 최소/권장 사양을 README에 정리했습니다.

      개발/검증 환경과 실제 권장사양을 구분해 표시했습니다.

   🧪 테스트 구조 정리

      테스트 프로젝트를 Core/Ocr, Core/Settings, Core/Text, Core/Translation 폴더로 나눠 메인 코드 구조와 맞췄습니다.

      테스트 파일 이동만 수행했으며 테스트 내용은 변경하지 않았습니다.

   🔌 번역 API 호출부 분리

      Google 번역, Gemini 모델 목록 조회, Gemini 번역 HTTP 호출을 TranslationApiClient로 분리했습니다.

      MainWindow는 기존 재시도, fallback, UI 로그 흐름을 유지하고 API 호출/응답 파싱 책임만 별도 클래스로 이동했습니다.

      가짜 HttpMessageHandler 기반 단위 테스트를 추가해 실제 네트워크 없이 API 호출부를 검증합니다.

      전체 단위 테스트 수가 164개로 늘어났습니다.
